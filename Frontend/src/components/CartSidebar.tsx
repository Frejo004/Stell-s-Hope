...
export default function CartSidebar() {
  const { cart, guestCart, isOpen, setIsOpen, cartItemsCount } = useCart();
            ...
            {cartItemsCount === 0 ? (
              ...
              <div className="space-y-4">
                {!isAuthenticated && guestCart.length > 0 ? (
                  // Affichage simplifié pour le panier invité
                  <div className="text-center py-8">
                  ...
                  // Affichage détaillé pour les utilisateurs connectés
                  cart.items?.map((item, index) => (
                    <div key={index} className="flex space-x-3 border-b pb-4">
                      ...
                      <div className="flex-1">
                        <h3 className="font-medium text-sm">Article {index + 1}</h3>
